...
{
    /// <summary>
    /// Facade WebAdmin cho câu trả lời bài thi: phân trang có lọc theo phiên thi/câu hỏi,
    /// ApiListHelper, nạp dropdown, map ViewModel ↔ SaveModel.
    /// </summary>
    public class ExamSessionAnswerService : IExamSessionAnswerService
    {
        // Repository ủy quyền gọi HTTP API câu trả lời bài thi
        private readonly IExamSessionAnswerRepository _repository;
        ...
        public ExamSessionAnswerService(
            IExamSessionAnswerRepository repository,
        ...
        /// </summary>
        public async Task<(List<ExamSessionAnswerModel> Items, long TotalItems, string? ErrorMessage, List<SelectListItem> ExamSessionFilterOptions, List<SelectListItem> QuestionFilterOptions)> GetListPagingAsync(int pageIndex, long? examSessionId, long? questionId)
        {
            var result = await _repository.GetListAsync(new ExamSessionAnswerSearchModel
            {
                PageIndex = pageIndex,
                PageSize = 10,
        ...
        public async Task<ExamSessionAnswerFormViewModel> BuildCreateFormAsync()
        {
            var model = new ExamSessionAnswerFormViewModel();
            await PopulateFormOptionsAsync(model);
            return model;
        }

        /// <summary>Lấy form sửa: map API → ViewModel (Mapster), nạp dropdown.</summary>
        public async Task<(ExamSessionAnswerFormViewModel? Model, string? ErrorMessage)> GetEditFormAsync(long id)
        {
            var result = await _repository.GetByIdAsync(id);
            if (!result.Success || result.Data == null)
                return (null, result.Message ?? "Không tìm thấy câu trả lời.");

            var model = result.Data.Adapt<ExamSessionAnswerFormViewModel>();
            await PopulateFormOptionsAsync(model);
            return (model, null);
        }

        /// <summary>Map ViewModel → SaveModel và gọi API thêm.</summary>
        public Task<ResponseData<object>> InsertAsync(ExamSessionAnswerFormViewModel model)
            => _repository.AddAsync(model.Adapt<ExamSessionAnswerSaveModel>());

        /// <summary>Map ViewModel → SaveModel và gọi API cập nhật.</summary>
        public Task<ResponseData<object>> UpdateAsync(ExamSessionAnswerFormViewModel model)
            => _repository.UpdateAsync(model.Adapt<ExamSessionAnswerSaveModel>());
        ...
        public async Task PopulateFormOptionsAsync(ExamSessionAnswerFormViewModel model)
        {
        ...
        /// <summary>Tải danh sách phiên thi làm SelectListItem (hiển thị thí sinh + bộ đề).</summary>
        public async Task<List<SelectListItem>> LoadExamSessionOptionsAsync(long selectedId)
        {
            ...
            return result.Success && result.Data != null
                ? result.Data.Select(e => new SelectListItem(
                    $"#{e.Id} - {e.UserFullName} / {e.ExamSetName}", e.Id.ToString(), e.Id == selectedId)).ToList()
